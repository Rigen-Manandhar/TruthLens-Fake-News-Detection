...
export default function AccountSummaryCard({
  ...
  return (
    <section className="rounded-3xl bg-[#fffdfa]/90 border border-(--line) p-5 sm:p-6 break-words">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {(profile?.providerInfo.providers ?? []).map((provider) => (
          <span
            key={provider}
            className="rounded-full border border-(--line) bg-[#fffdf8] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-(--muted-foreground)"
          >
            {provider}
          </span>
        ))}
