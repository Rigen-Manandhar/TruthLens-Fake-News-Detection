...
export default function Footer({ className = "mt-16" }: FooterProps) {
  ...
  return (
    <footer
      className={`${className} rounded-4xl border border-(--line) bg-[#fffdf8]/90 px-6 py-8 shadow-[0_14px_34px_rgba(20,16,8,0.08)]`}
    >
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)]">
        <div>
          <div className="display-title text-xl font-bold text-[#17130f]">
            TruthLens
          </div>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-xs font-semibold text-[#736759]">
            {productLinks.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="transition-colors hover:text-[#12100d]"
                >
                  {link.label}
                </Link>
              </li>
      ...
      <div className="mt-6 flex flex-col items-center justify-between gap-3 border-t border-(--line)/70 pt-4 text-xs text-[#8a7d6d] sm:flex-row">
